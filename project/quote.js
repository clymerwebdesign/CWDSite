...
(function () {
  ...
  var I = {
    food:   '<path d="M3 2v7a3 3 0 0 0 3 3v10M6 2v6M9 2v6M9 2v7a3 3 0 0 1-3 3M17 2c-1.5 0-3 1.8-3 5s1 5 3 5v8" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>',
    beauty: '<path d="M6 9a3 3 0 1 0 0-6 3 3 0 0 0 0 6ZM6 21a3 3 0 1 0 0-6 3 3 0 0 0 0 6ZM20 4 8.1 16M14.5 12.5 20 20" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>',
    trades: '<path d="M14.7 6.3a4 4 0 0 0-5.2 5.2l-6 6a1.5 1.5 0 0 0 2.1 2.1l6-6a4 4 0 0 0 5.2-5.2l-2.5 2.5-2.1-2.1 2.5-2.5Z" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>',
    retail: '<path d="M6 2 3 6v14a1 1 0 0 0 1 1h16a1 1 0 0 0 1-1V6l-3-4ZM3 6h18M16 10a4 4 0 0 1-8 0" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>',
    org:    '<path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2M9 11a4 4 0 1 0 0-8 4 4 0 0 0 0 8ZM23 21v-2a4 4 0 0 0-3-3.87M16 3.13a4 4 0 0 1 0 7.75" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>',
    spark:  '<path d="M12 3v4M12 17v4M3 12h4M17 12h4M6.3 6.3 9 9M15 15l2.7 2.7M17.7 6.3 15 9M9 15l-2.7 2.7" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>',
    ...
    globe:  '<path d="M12 21a9 9 0 1 0 0-18 9 9 0 0 0 0 18ZM3 12h18M12 3a14 14 0 0 1 0 18 14 14 0 0 1 0-18Z" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>',
    advice: '<path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2v10Z" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>',
  ...
  var STEPS = [
    {
      key: 'businessType', type: 'single',
      q: 'What kind of business do you have?',
      sub: "No wrong answer — this just helps me picture your site.",
      chatQ: "First up — what kind of business are we putting online?",
      options: [
        { v: 'food',   label: 'Restaurant / Food & drink', icon: 'food' },
        { v: 'beauty', label: 'Health & beauty',           icon: 'beauty' },
        { v: 'trades', label: 'Trades & services',         icon: 'trades' },
        { v: 'retail', label: 'Retail & shops',            icon: 'retail' },
        { v: 'org',    label: 'Organization / non-profit', icon: 'org' },
        { v: 'else',   label: 'Something else',            icon: 'spark' }
      ]
    },
    {
      key: 'needs', type: 'multi',
      q: 'What are you looking for?',
      sub: 'Pick anything that fits — choose as many as you like.',
      chatQ: "Got it! And what are you after? (pick as many as you want)",
      options: [
        { v: 'new',     label: 'A brand new website',          icon: 'plus' },
        { v: 'replace', label: 'Replace an old website',       icon: 'refresh' },
        { v: 'domain',  label: 'Just need it on a domain',     icon: 'globe' },
        { v: 'advice',  label: 'Not sure — need advice',       icon: 'advice' }
      ]
    },
    {
      key: 'pages', type: 'single',
      q: 'How many pages do you think you need?',
      sub: "A rough guess is totally fine — we'll refine it together.",
      chatQ: "Roughly how big are we talking? (pages-wise)",
      options: [
        { v: 's',  label: '1–3 pages',  icon: 'pages' },
        { v: 'm',  label: '4–6 pages',  icon: 'pages' },
        { v: 'l',  label: '7–10 pages', icon: 'pages' },
    ...
    },
    {
      ...
      sub: 'Optional add-ons. Skip any you don\'t need.',
      chatQ: "Almost there! Want any of these extras thrown in?",
      options: [
        ...
        { v: 'setup',   label: 'Custom domain setup',   icon: 'globe' },
        { v: 'unsure',  label: "I'm not sure yet",      icon: 'help' }
      ]
    }
  ];

  var BASE = { s: [150,250], m: [300,450], l: [500,700], '?': [200,450] };

  /* ---------- state ---------- */
  var state = {
    mode: 'guided',
    step: 0,
    answers: { businessType: null, needs: [], pages: null, extras: [] }
  };

  function estimate() {
    var base = BASE[state.answers.pages] || BASE['?'];
    var extras = state.answers.extras.filter(function (e) { return e !== 'unsure'; });
    var lo = base[0] + extras.length * 25;
    var hi = base[1] + extras.length * 50;
    return { lo: lo, hi: hi, extras: extras.length };
  }
  function answeredCurrent() {
    var s = STEPS[state.step];
    var a = state.answers[s.key];
    return s.type === 'single' ? !!a : (a && a.length > 0);
  }
  ...
  var guided = {
    render: function (dir) {
      ...
      var html =
        ...
        '<div class="g-panel"><div class="g-slide" id="g-slide">' +
          ...
          '<div class="g-options ' + (s.options.length <= 4 && s.options.length % 2 ? 'cols-1' : '') + '">' + opts + '</div>' +
          ...
          '<div class="g-actions">' +
            ...
            (multi
              ? '<button class="btn btn-navy g-continue" ' + (sel.length ? '' : 'disabled') + '>Continue <span class="arrow">→</span></button>'
              : '<span></span>') +
          '</div>' +
        '</div></div>';
    ...
    bind: function () {
      ...
      if (back) back.addEventListener('click', function () {
        if (state.step > 0) { state.step--; guided.render('back'); }
      });
    },
    advance: function () {
  ...
  var chat = {
    ...
    render: function () {
      stage.querySelector('#q-card').innerHTML =
        '<div class="c-window">' +
          '<div class="c-head">' +
            ...
            '<div><div class="c-head-name">Greyson Clymer</div>' +
            '<div class="c-head-status">Online now</div></div>' +
          '</div>' +
      ...
      if (!chat.started) {
        chat.started = true;
        chat.botSay("Hey there! 👋 I'm Greyson. Let's build you a quick estimate — should only take a minute.", function () {
          chat.askStep();
        });
      } else {
        chat.replay();
    ...
    replay: function () {
      // re-render conversation up to current step (when switching back to chat)
      for (var i = 0; i < state.step; i++) {
        ...
        var txt = s.type === 'single'
          ? labelFor(s, a)
          : a.map(function (v) { return labelFor(s, v); }).join(', ');
    ...
    next: function (withAck) {
      ...
      var acks = ['Perfect.', 'Nice one!', 'Got it.', 'Love it.', 'Great choice!'];
      if (state.step < STEPS.length - 1) {
        state.step++;
        if (withAck) {
  ...
  function showResult() {
    ...
    var pagesLabel = labelFor(STEPS[2], state.answers.pages) || 'Custom';
    stage.querySelector('#q-card').innerHTML =
      '<div class="r-wrap">' +
        '<p class="r-eyebrow">Your estimate</p>' +
        '<h2 class="r-title">Here\'s your ballpark</h2>' +
        '<div class="r-estimate">' +
          '<div class="r-est-label">Estimated project range</div>' +
          '<div class="r-est-price">' + money(e.lo) + '<span class="dash">–</span>' + money(e.hi) + '</div>' +
          '<div class="r-est-meta"><span>' + pagesLabel + '</span>' + extraNote + '</div>' +
        '</div>' +
        '<p class="r-note">This is just an estimate — final pricing is confirmed before any work begins. Send your details and I\'ll get back to you within a day.</p>' +
        '<form class="r-form" id="q-form" netlify name="quote" method="POST" data-netlify="true">' +
          '<input type="hidden" name="form-name" value="quote">' +
          '<input type="hidden" name="estimate" value="' + money(e.lo) + ' – ' + money(e.hi) + '">' +
    ...
    form.addEventListener('submit', function (ev) {
      ev.preventDefault(); // prototype: simulate success. (Netlify attrs left in place for live deploy.)
  ...
  function restart() {
    state.step = 0;
    state.answers = { businessType: null, needs: [], pages: null, extras: [] };
    chat.started = false;
    mount();
  }
